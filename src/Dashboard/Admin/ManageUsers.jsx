          ...
          <table className="table table-zebra">
            ...
            <tbody>
              {users.map((user, index) => <tr key={user._id}>
                ...
                <td>
                  {user?.role === 'instructor' || user?.email == 'ashrafulfahim07@gmail.com' ? <button className='btn btn-xs btn-neutral 'disabled >Make Instructor</button> : <button className='btn btn-xs' onClick={() => makeInstructor(user)} >Make Instructor</button>}
                </td>
                <td>
                  {user?.role === 'admin' && user?.email === 'ashrafulfahim07@gmail.com' ? <button className='btn btn-xs  btn-neutral' disabled>Remove</button> : <button className='btn btn-xs' onClick={() => reMoveUSer(user)} >Remove</button>}
                </td>
              </tr>)
